...
#                          CONFIG
# ============================================================
# These are only FALLBACK defaults, used when this script is run
# directly (python image_extract.py) with no arguments. For
# programmatic use (e.g. from a Streamlit app, or any other caller),
# pass values into main(...) instead — see the main() signature below.
# DOC_PATH has no hardcoded personal path anymore: it must be provided
# either via main(doc_path=...) or by editing this CONFIG block before
# a direct run.
DOC_PATH = None
...
def main(doc_path=None, output_dir=None, target_section=None,
         image_name=None, text_name=None, xlsx_name=None,
         png_scale=None, extract_diagram_image=None,
         extract_reference_images=None, pin_map=None):
    """
    Run the full extraction pipeline.

    Every argument is optional and falls back to the CONFIG block
    above when omitted — so `python image_extract.py` with CONFIG
    edited still works exactly as before. But any caller (a Streamlit
    app, a test script, another pipeline) can now call this directly
    with explicit values instead of needing to reach into the module
    and overwrite its globals first:

        import image_extract
        image_extract.main(
            doc_path="uploaded.docx",
            output_dir="output/run_123",
            target_section="4.1.1",
        )

    Parameters
    ----------
    doc_path : str or Path       — path to the source .docx (required,
                                    either here or via CONFIG.DOC_PATH)
    output_dir : str or Path     — where results are written
    target_section : str         — section number containing the diagram
    image_name : str             — base filename for the diagram image
    text_name : str              — filename for the diagram text output
    xlsx_name : str               — filename for the combined Excel output
    png_scale : int              — EMF->PNG export scale factor
    extract_diagram_image : bool — whether to export the section diagram
    extract_reference_images : bool — whether to export per-pin images
    pin_map : dict                — label -> MarkVIe pin name overrides
    """
    global DOC_PATH, OUTPUT_DIR, TARGET_SECTION, IMAGE_NAME, TEXT_NAME
    global XLSX_NAME, PNG_SCALE, EXTRACT_DIAGRAM_IMAGE
    global EXTRACT_REFERENCE_IMAGES, PIN_MAP
    ...
    if png_scale is not None:
        PNG_SCALE = png_scale
    if extract_diagram_image is not None:
        EXTRACT_DIAGRAM_IMAGE = extract_diagram_image
    if extract_reference_images is not None:
    ...
    if not DOC_PATH:
        raise SystemExit(
            "No document path provided. Pass main(doc_path=...) or set "
            "DOC_PATH in the CONFIG block before running directly."
        )

    print("Document : %s" % DOC_PATH)
    if not Path(DOC_PATH).exists():
        raise SystemExit("Document not found! Check DOC_PATH: %s" % DOC_PATH)
